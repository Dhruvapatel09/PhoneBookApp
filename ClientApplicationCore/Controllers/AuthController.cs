...
namespace ClientApplicationCore.Controllers
{
    ...
    public class AuthController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> ForgotPassword(ForegetViewModel viewModel)
        {
            if (ModelState.IsValid)
            {
            ...
            if (ModelState.IsValid)
            {
                if (updateContact.FormFile != null && updateContact.FormFile.Length > 0)
                {
                    ...
                    using (var memoryStream = new MemoryStream())
                    {
                        ...
                        updateContact.File = memoryStream.ToArray();
                    }
        ...
        {
            ForegetViewModel viewModel = new ForegetViewModel();
            viewModel.UserName = @User.Identity.Name;
            return View(viewModel);
        }
        [HttpPost]
        public IActionResult ChangePassword(ForegetViewModel viewModel)
        {
            if (ModelState.IsValid)
            {
                var apiUrl = $"{endPoint}Auth/ForgetPassword";
